...
class Training(CreatableFromParameters):
	def __init__(
		...
		perception_processor_type: str, simulation_type: str, perception_nodes: list[str],
		...
		self.poisonous_perception_distance	: Optional[int]			= None
	...
	def generate_perception_processor_parameter(self) -> dict[str, Any]:
		...
		params : dict[str, Any] = {
			"perception-nodes"	: self.perception_nodes
		}
		if self.normalized != None:
			params["normalized"] = self.normalized
		if self.n_cones != None:
			params["n-cones"] = self.n_cones
		if self.fov != None:
			params["fov"] = self.fov
		if self.see_agents != None:
			params["see-agents"] = self.see_agents
		if self.see_food != None:
			params["see-food"] = self.see_food
		if self.see_poisonous_food != None:
			params["see-poisonous-food"] = self.see_poisonous_food
		if self.see_walls != None:
			params["see-walls"] = self.see_walls
		if self.n_freq != None:
	...
	def to_dict(self) -> dict[str, Any]:
		...
		if self.poisonous_perception_distance != None: data |= {"poisonous-perception-distance" : self.poisonous_perception_distance}
